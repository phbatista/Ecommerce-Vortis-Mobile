    ...
    try {
    const resp = await fetch("http://localhost:8080/api/vendas/admin"); // endpoint de listagem
    ...
    if (!confirmacao) return;
        ...
        botao.onclick = () => {
            const novoStatus = select.value;
            fetch(`http://localhost:8080/api/vendas/admin/${p.id}/status?status=${encodeURIComponent(novoStatus)}`, {
                method: "PUT"
            })
